...
- For handlers that always need MongoDB (and the shared preset), use **`createGlobalRouteHandler`** from `@/app/api/_shared/route-handlers/global-route-handler.util` instead of wiring `withMongoDbConnection` by hand. That preset also applies **`withRouteErrorHandler`** (outermost) for structured JSON error responses (`error`, optional `error_code`, optional `snackbar`); see [lib/http/README.md](../http/README.md).
- Shared DB typings: `ISchemaDefinition` in `DB/interfaces`. Per-entity document interfaces live next to their schema (e.g. `IUserSchema` in `DB/schemas/user`, `ISessionSchema` in `DB/schemas/session`). The path alias `DB/*` → `lib/mongodb/*` is set in `tsconfig.json`.
- Mongoose `Schema` instances and definition objects such as `UserSchemaDefinition` / `SessionSchemaDefinition` live under `DB/schemas`.
...
To use the definition directly (e.g. another framework), import `UserSchemaDefinition` or `SessionSchemaDefinition` from `DB/schemas`.
