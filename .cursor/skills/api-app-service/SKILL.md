...
| [app/api/\_shared/route-handlers/with-validated-body-route-middleware.util.ts](../../../app/api/_shared/route-handlers/with-validated-body-route-middleware.util.ts) | **`withValidatedBody(schema)`** — Zod **`safeParse`** on JSON body → **`context.body`**; **`BadRequestError`** (**`"Invalid request"`**) on failure; single **`request.json()`** for the chain. |
| [app/api/\_shared/route-handlers/with-validated-query-route-middleware.util.ts](../../../app/api/_shared/route-handlers/with-validated-query-route-middleware.util.ts) | **`withValidatedQuery(schema)`** — Zod on **`searchParams`** → **`context.query`**; same **`BadRequestError`** on failure. |
...
| [src/interfaces/api-error-body.interface.ts](../../../src/interfaces/api-error-body.interface.ts) | **`IApiErrorBody`** — shared **`error`**, optional **`error_code`**, optional **`snackbar`** shape for **`jsonErrorBody`** and client **`FetchApiError`**. |
