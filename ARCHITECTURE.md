...
- **`AppError`** in `app/api/_shared/features/error-handling/instances/app-error.ts` is a typed error class (optional `statusCode`, `code`) routes may throw; **`withRouteErrorHandler`** treats it distinctly when logging.
- **`withRouteErrorHandler`** in `app/api/_shared/features/error-handling/middlewares/with-route-error-handler-route-middleware.util.ts` is app-level middleware for consistent JSON error responses on failures (used as the outermost layer in the preset below).
