...
| Path                         | Purpose |
| ---------------------------- | ------- |
| `src/features/error-handling/` | Shared API error **enums**, **`APP_ERROR_CODES_TYPE`**, and snackbar payload **`ISnackbarArgs`** (**`interfaces/snackbar-args.interface.ts`**) used with **`AppError`** / route error middleware (see §4). The HTTP error JSON shape **`IApiErrorBody`** is shared with the client via **`src/interfaces/api-error-body.interface.ts`** (**`jsonErrorBody`**, **`FetchApiError.body`**). |
